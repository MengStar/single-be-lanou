...
- 保持及时更新,新增特性及时修改此文档
- 开发环境下运行阅读 `Getting started`
- 生产环境运行`npm run build`会生成`build`文件夹，打开下面的`index.html`
- 组件的说明从官网摘录，没有翻译
...
The default username is `Admin` and the default password is `Admin`, but feel free to register new users! The registered users are saved to localStorage, so they'll persist across page reloads.
...
1. Clone this repo using `git clone git@github.com:MengStar/team-fe.git`.
...
* `components`: The main JS folder. All the React components are in this folder.

* `pages`: (routes) saved in the folder. E.g. a navigation component `Nav.react.js`
...
* `store`: The store folder contains every state creating from reducers.
...
Please read the `makewebpackconfig.js` `webpack.dev.config.js` `webpack.prod.config.js` for[webpack](http://webpack.github.io) and `.babelrc` for [babel](http://babeljs.io).

### npm

Please read the `package.json` for [npm](http://www.ruanyifeng.com/blog/2016/01/npm-install.html)
